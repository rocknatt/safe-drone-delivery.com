...
class RatchetClient
{
     ...
     * @var array
     */
    protected $callback_type = array('auth', 'event', 'close');
...
class Server implements MessageComponentInterface
{
    ...
    */
    ...
    /**
     * Client ratchet how call the instance
     * @var RatchetClient
     */
    protected $ratchet_client;
     ...
     * @author Romain GALLIEN <romaingallien.rg@gmail.com>
     */
    public function __construct($ratchet_client)
    {
        $this->ratchet_client = $ratchet_client;

        // Initialize object as SplObjectStorage (see PHP doc)
        $this->clients = new SplObjectStorage;

        // // Check if auth is required
        if ($this->ratchet_client->auth && empty($this->ratchet_client->callback['auth'])) {
            output('fatal', 'Authentication callback is required, you must set it before run server, aborting..');
        }

        // Output
        if ($this->ratchet_client->debug) {
            output('success', 'Running server on host '.$this->ratchet_client->host.':'.$this->ratchet_client->port);
        }

        // Output
        if (!empty($this->ratchet_client->callback['auth']) && $this->ratchet_client->debug) {
    ...
    public function onOpen(ConnectionInterface $connection)
    {
        ...
        $connection->subscriber_id = array('token' => '', 'id' => 0);
    ...
    public function onMessage(ConnectionInterface $client, $message)
    {
        ...
        if (!empty($datas->token) && $datas->token !== $client->resourceId) {

            // At this moment we have to check if we have authent callback defined
            if (!empty($this->ratchet_client->callback['auth']) && empty($client->subscriber_id['token'])) {

                // Call user personnal callback
                $auth = call_user_func_array($this->ratchet_client->callback['auth'], array($datas));
        ...
        if (!empty($message)) {
            ...
            $this->broadcast($datas, $message, $client, $broadcast);
        }
    }

    public function broadcast($datas, $message, $client, $broadcast = true)
    {
        ...
        foreach ($this->clients as $user) {
            ...
            if (!empty($datas->recipient_id)) {
                // var_dump($datas->recipient_id . ' ' . $message . ' ' . $user->subscriber_id['id']);
                if ($user->subscriber_id['id'] == $datas->recipient_id && $user->subscriber_id['token'] != $datas->token) {
    ...
    public function onClose(ConnectionInterface $connection)
    {
        ...
        if (!empty($this->ratchet_client->callback['close'])) {

            // At this moment we have to check if we have authent callback defined
            call_user_func_array($this->ratchet_client->callback['close'], array($connection, $this->clients, $this));
        }
    ...
    {
        var_dump($user->subscriber_id['id'], $message);
        // Send the message
        $user->send($message);

        // We have to check if event callback must be called
        if (!empty($this->ratchet_client->callback['event'])) {

            // At this moment we have to check if we have authent callback defined
            call_user_func_array($this->ratchet_client->callback['event'], array((valid_json($message) ? json_decode($message) : $message), $user, $this->clients, $this));

            // Output
            if ($this->ratchet_client->debug) {
                // output('info', 'Callback event "'.$this->ratchet_client->callback['event'][1].'" called');
            }
        }
